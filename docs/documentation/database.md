...
geographically close to the primary instance, so LaunchFast has configured
...
When you initialize a LaunchFast app, it will ask you which region you wish to
...
deployment process. You can find this in the `other/litefs.yml` file.

We deploy to multiple instances at once and the way we deploy means we don't
have any downtime during deployments. However, to make this work, you do need to
...
running. In LaunchFast apps, that's normally just the currently running version
of the app. So you only need to worry about "breaking schema changes" whenever
you deploy a new version, but you don't have to worry about avoiding breaking
schema changes for old versions of the app that are no longer running.
...
of how this works (each step here is an individual deployment):

1. Widen app to consume B if available, otherwise fallback to A (which it has
   to, at this point)
2. Widen db to have both A and B (which is populated, during deployment, from A,
   another field, or a default value) and the app to write to both A and B
...
(again, each of these steps end in a deployment):

1. Widen the app to read from `firstName` and `lastName` if available (they
   won't be available at this point) or `name` as a fallback. So all the code
   ...
   exist yet.
2. Widen db to have `firstName` and `lastName`, in addition to `name`. The
   `firstName` field should be populated with the `name`. You can populate
   `firstName` as part of the migration SQL script. The easiest way to do this
   is to run `prisma migrate` to generate the migration SQL script that adds the
   `firstName` and `lastName` fields and then manually modify the script to copy
   the existing data in the `name` field to the `firstName` field (maybe with
   the help of GitHub Copilot 😄).
3. Narrow the app to only read from and write to `firstName` and `lastName`,
   removing the fallback to the `name` field.
4. Narrow db by removing the `name` field from the db schema.
...
Not all of us are excited about writing raw SQL (especially if what you need to
seed is a lot of data), so here's an easy way to help out:
...
2. Create a temporary database file to seed the data into.
   ```sh
   DATABASE_URL=file:./seed.local.db npx prisma migrate reset --skip-seed --force
   ```
3. Run the custom seed script locally to generate the data you want to seed.
   ```sh
   DATABASE_URL=file:./seed.local.db npx tsx ./prod-seed.local.ts
   ```
4. Create a "dump" of the seed database using the `sqlite3` command line tool.
   ```sh nonumber
   sqlite3 ./prisma/seed.local.db .dump > seed.local.sql
   ```
5. Copy the relevant bits from the `seed.local.sql` file into your
   `migration.sql` file. The `seed.local.sql` will include `create table/index`
   lines, which should already be in your `migration.sql`. You probably just
   need to copy the `INSERT` commands.
6. Deploy your app and verify that the data was seeded correctly.
...
2. Create a new database file (locally) with the data you want to seed.
3. Create a "dump" of the seed database using the `sqlite3` command line tool.
   ```sh nonumber
   sqlite3 seed.db .dump > seed.sql
   ```
4. Copy the `seed.sql` file to your production volume next to your database (via
   `fly sftp`)
5. SSH into your production server and run the following command:
   ```sh nonumber
   sqlite3 /tmp/data.db < seed.sql
   ```
6. Import the data into your database via litefs:
   ```sh nonumber
   litefs import -name sqlite.db /tmp/data.db
   ```
7. Verify that your production database has been seeded correctly. If it hasn't,
...
the machine that has the primary lease can start up, and then you can SSH into
it and fix the DB.
